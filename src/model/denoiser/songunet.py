...
class SongUNetCfg:
    ...
    attn_resolutions: List[int]  # Resolutions with self-attention.
    dropout: float  # Dropout probability of intermediate activations.
    label_dropout: (
        float  # Dropout probability of class labels for classifier-free guidance.
    )
    embedding_type: Literal["positional", "fourier"]  # Timestep embedding type.
    channel_mult_noise: (
        int  # Multiplier for the number of channels in the noise embedding.
    )
    encoder_type: Literal["standard", "skip", "residual"]  # Encoder architecture.
    decoder_type: Literal["standard", "skip"]  # Decoder architecture.
    resample_filter: List[int]  # Resampling filter for upsampling/downsampling.
...
class SongUNet(nn.Module):
    def __init__(
        ...
        assert cfg.embedding_type in ["fourier", "positional"]
        assert cfg.encoder_type in ["standard", "skip", "residual"]
        assert cfg.decoder_type in ["standard", "skip"]

        super().__init__()
        self.cfg = cfg

        self.label_dropout = cfg.label_dropout
        emb_channels = cfg.model_channels * cfg.channel_mult_emb
        noise_channels = cfg.model_channels * cfg.channel_mult_noise
        ...
        self.map_label = (
            Linear(in_features=cfg.label_dim, out_features=noise_channels, **init)
            if cfg.label_dim
            else None
        )
        self.map_augment = (
            Linear(
                in_features=cfg.augment_dim, out_features=noise_channels, bias=False, **init
            )
            if cfg.augment_dim
            else None
        )
        ...
        if cfg.activate_w_mode:
            self.map_w = nn.Sequential(
                PositionalEmbedding(num_channels=noise_channels, endpoint=True),
                Linear(in_features=noise_channels, out_features=noise_channels, **init),
            )

        # Encoder.
        self.enc = nn.ModuleDict()
        cout = cfg.in_channels
        caux = cfg.in_channels
        for level, mult in enumerate(cfg.channel_mult):
            res = cfg.img_resolution >> level
            if level == 0:
                cin = cout
                cout = cfg.model_channels
            ...
            for idx in range(cfg.num_blocks):
                cin = cout
                cout = cfg.model_channels * mult
                attn = res in cfg.attn_resolutions
        ...
        for level, mult in reversed(list(enumerate(cfg.channel_mult))):
            ...
            if cfg.decoder_type == "skip" or level == 0:
                if cfg.decoder_type == "skip" and level < len(cfg.channel_mult) - 1:
                ...
                self.dec[f"{res}x{res}_aux_conv"] = Conv2d(
                    in_channels=cout, out_channels=cfg.out_channels, kernel=3, **init_zero
                )

    def forward(self, sample, timestep, class_labels=None, w=None, augment_labels=None, return_dict=True):
        # Mapping.
        ...
        if self.cfg.activate_w_mode:
            if not torch.is_tensor(w):
                w = torch.tensor([w], dtype=torch.float32, device=sample.device)
            elif torch.is_tensor(w) and len(w.shape) == 0:
                w = w[None].to(sample.device)
